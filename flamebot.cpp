...
int main() {
    ...
    while(1){
        ...
        float s1 = ir_R.read() * 1023; // fire intensity
        float s2 = ir_M.read() * 1023; // fire intensity
        float s3 = ir_L.read() * 1023; // fire intensity

        printf("Right: %0.0f, Front: %0.0f, Left: %0.0f\r\n", s1, s2, s3); // print sensor values for debugging
        ThisThread::sleep_for(50); // small delay
        ...
        if (s1 < 350) { // was 250 -fire detected on the right
            turnRight(); // face fire
            ...
            // activate pump & servo 
            extinguish();
        } else if (s2 < 400) { // was 350 - fire detected in front
            forward();
            ThisThread::sleep_for(500);
            stop();
            // activate pump & servo 
            extinguish();
        } else if (s3 < 350) { // was 250 - fire detected on the left
            turnLeft(); // face fire
            ThisThread::sleep_for(400); // wait to turn
            stop(); 
            forward(); // move forward after turning
